...
# on introduit une modification ici
...
times_by_incline <- function (graph, wt_profile) {
  ...
  if (wt_profile == "foot") {
    walkbler <- approxfun(
      x = c(-.3, -.2, -.15, -.1, -.05, 0, .05, .1, .15, .2, .3 ),
      y = c(3, 1.4, 1.2 , 1, 0.85, 1, 1.2, 1.41, 1.7, 2, 4)
    )
    
    if ("dz" %in% names (graph)) {
      graph$time <- graph$time * walkbler (graph$dz/graph$d)
      graph$time_weighted <- graph$time_weighted * walkbler (graph$dz/graph$d)^2
    }
    
  } else if (wt_profile == "bicycle") {
    bikbler <- approxfun(
      x = c(-.3, -.2, -.15, -.1, -.05, 0, .05, .1, .15, .2, .3 ),
      y = c(3,1.4, 1 , 0.75, 0.8, 1, 1.2, 1.41, 1.7, 2, 4)
    )
